...
from src.basedatos import GestorBaseDatos as bd
...
def construir_preprocesador():
    cat_cols, num_cols = definir_columnas()
...
def __entrenar_modelo__(nombre_modelo, X, y):
    preprocessor = construir_preprocesador()
...
def procesar_modelo_ml():
    ...
    try:
        ...
        mejor = __analisis_modelo__(X, y)
        modelo = __entrenar_modelo__(mejor, X, y)
    finally:
...
def predecir_nuevo(modelo_entrenado, provincia, canton, distrito, tipo_ruta,
    ...
    for clase, p in zip(clases, prob):
        print(f"Probabilidad de {clase}: {p:.4f}")
