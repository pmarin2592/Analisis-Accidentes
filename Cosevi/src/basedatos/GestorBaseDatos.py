...
def config(filename = os.path.abspath(os.path.join(os.path.dirname(__file__), '../config.ini')), section='postgresql'):
    parser = ConfigParser()
...
def conectar(max_reintentos=20, espera_segundos=3):
    conn = None
    for intento in range(1, max_reintentos + 1):
        try:
            ...
            if intento == max_reintentos:
                print("❌ Error al conectar: connection to server at \"40.76.114.77\", port 5432 failed: Connection timed out (0x0000274C/10060)\n¿Está el servidor ejecutándose y aceptando conexiones TCP/IP?")
                return None
            time.sleep(espera_segundos)  # Espera entre reintentos

def carga_acidentes_victimnas():
    ...
    chunk_size = 1000
    chunks = list(dividir_en_chunks(values, chunk_size))
...
def actualizar_ubicaciones(id, longitud, latitud):
    conn = None
    cursor = None
    try:
        conn = conectar()
...
def insertar_lluvia(provincia, canton, distrito, dia_semana, mes, anio, hora, lluvia_acumulada):
    conn = None
    cursor = None
    try:
        conn = conectar()
